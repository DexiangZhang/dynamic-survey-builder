...
const QuestionAddModal = ({ onClose, onSave }: any) => {
  const [currentDraft, setCurrentDraft] = useState<any>(createInitialDraft);

  const handleInternalUpdate = (key: any, value: any) => {
  ...
  const handleUpdateOptionText = (optionId: any, newText: any) => {
    setCurrentDraft((prev: any) => ({
      ...prev,
  ...
  const handleSaveAndClose = () => {
    if (!currentDraft?.name || currentDraft?.name?.trim() === "") return;
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-4 border-b pb-3">
        <p className="text-lg font-medium text-black">Add New Question</p>
        <button
          onClick={onClose}
      ...
      <div className="mt-6 pt-4 border-t flex justify-between">
        ...
        <button
          ...
          className={`py-2 px-4 text-white font-semibold rounded-lg shadow-md transition-colors text-sm ${
            isSaveDisabled
